(function() {
  ...
  const { currentPlayer, smallBlindPlayer: prevSmallBlindPlayer } = round;
  ...
  case 'BET': {
    const { zone_turn: zoneTurn, zone_river: zoneRiver } = decks;
    ...
    if (allPlayersReady) {
      ...
      case 'river': {
        ...
        const { bestOffer } = this.selectBestOffer(offersMap);
        const { player: winner, carCard, serviceCards } = bestOffer;
