  ...
  switch (this.roundStep) {
    case 'ROUND_START': {
      ...
      round.currentPlayer.activate({
        ...
        setData: { eventData: { controlBtn: { label: 'Test', triggerEvent: null } } },
      });
        ...
        switch (round.step) {
          case 'preflop':
            ...
            function addOffer(player, car, serviceCards) {
              const offerKey =
                car.id() + ('+' + serviceCards.length ? serviceCards.map((c) => c.name).join('+') : '');
            ...
            const {
              bestOffer: { player, carCard: { title: carTitle } = {}, serviceCards, price },
              offersCount,
            } = this.selectBestOffer(offersMap);
