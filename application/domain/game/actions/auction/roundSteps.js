(function () {
  ...
  //   const { replacedClientDeal } = round;
  ...
  const initDealOffersStep = (player) => {
    ...
    player.activate({
      notifyUser:
        'Выбери клиента и сделай ему предложение о покупке авто (рекомендуется добавлять только сервисы, меняющие характеристики авто, так как увеличить цену сделки можно будет на шаге дополнительных продаж).',
      setData: { eventData: { controlBtn: { label: 'Подтвердить выбор' } } },
    });
  ...
  switch (this.roundStep) {
    ...
    case 'CHECK_DEAL': {
      ...
      if (offersCount > 0) {
        ...
        if (round.featureCard.replaceClient && !round.featureCard.played) {
          ...
          result.newRoundLogEvents.push(
            `Произошла замена клиента. Расчет сделки ведется исходя из бюджета "${newClient.title}", но по требованиям "${clientCard.title}".`
          );
          ...
          const [{ player: dealPlayer } = {}] = offerValues;

          returnCardsToOwners(dealDeck);

          return initDealOffersStep(dealPlayer);
        }
      }
    ...
    case 'PRESENT': {
      ...
      player.activate({
        notifyUser:
          'Ты можешь сделать дополнительные продажи (не превышающее в сумме стоимость авто). При превышении бюджета клиента сделка будет отменена.',
